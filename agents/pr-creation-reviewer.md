...
1. **Identify the PR**: Determine the PR number or URL from the context provided. If a PR was just created via `gh pr create`, extract the PR number or URL from the output of that command.

2. **Read the skill instructions**: Read the file at `~/.claude/skills/review-pr` to understand the exact review process and instructions defined there.
...
- Always read `~/.claude/skills/review-pr` first before doing anything else. The skill file is the source of truth for how reviews should be conducted.
- If the skill file cannot be found or read, report this clearly and explain that the review cannot proceed without it.
- If you cannot determine which PR to review, use `gh pr list --author @me --state open --limit 1` to find the most recently created PR.
